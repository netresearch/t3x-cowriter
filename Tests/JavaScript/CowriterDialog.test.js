...
describe('CowriterDialog', () => {
    ...
    describe('_appendStatusLink', () => {
        it('should render a link when statusUrl is provided', async () => {
            const dialog = new CowriterDialog(mockService);
            ...
            expect(link).not.toBeNull();
            expect(link.href).toContain('/typo3/module/cowriter/status');
            expect(link.textContent).toContain('Open Setup Status');
            expect(link.target).toBe('_blank');
            expect(link.rel).toBe('noopener noreferrer');
        });

        it('should fall back to getModuleUrl when statusUrl is falsy', async () => {
        ...
        it('should not render a link when neither statusUrl nor getModuleUrl provides a URL', async () => {
            const dialog = new CowriterDialog(mockService);
            const container = document.createElement('div');
    ...
    describe('status link in error flows', () => {
        it('should show status link when result has statusUrl but no content', async () => {
            mockService.executeTask.mockResolvedValue({
                success: false,
                error: 'LLM is not configured yet.',
                statusUrl: '/typo3/module/cowriter/status',
            });
            ...
            await vi.waitFor(() => {
                const preview = document.querySelector('[data-role="result-preview"]');
                return preview.textContent.includes('LLM is not configured yet.');
            });
        ...
        it('should show status link when executeTask throws error with statusUrl', async () => {
            const err = new Error('No default provider configured');
            err.statusUrl = '/typo3/module/cowriter/status';
            mockService.executeTask.mockRejectedValue(err);
            ...
            await vi.waitFor(() => {
                const preview = document.querySelector('[data-role="result-preview"]');
                return preview.textContent.includes('No default provider configured');
            });
